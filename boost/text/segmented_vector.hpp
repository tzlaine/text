...
namespace boost { namespace text {
    ...
    struct segmented_vector
    {
        ...
        segmented_vector & operator= (segmented_vector && rhs) noexcept = default;
        ...
        {
            detail::foreach_leaf(ptr_, [&](detail::leaf_node_t<T> const * leaf) {
                switch (leaf->which_) {
                ...
                case detail::leaf_node_t<T>::which::ref:
                    f(
                        &*leaf->as_reference().vec_.as_leaf()->as_vec().begin(),
                        &*leaf->as_reference().vec_.as_leaf()->as_vec().end()
                    );
                    break;
        ...
        /** Returns true if *this and rhs contain the same root node pointer.
            ...
            segmented_vectors that are likely to have originated from the same initial
            segmented_vector, and may have since been mutated. */
        ...
        /** Replaces the portion of *this delimited by [old_first, old_last)
           ...
           \pre begin() <= old_substr.begin() && old_substr.end() <= end() */
        template <typename Iter>
        segmented_vector & replace (const_iterator old_first, const_iterator old_last, Iter new_first, Iter new_last)
    ...
    private:
        ...
        vec_insertion mutable_insertion_leaf (iterator at, size_type size, allocation_note_t allocation_note)
        {
            if (!ptr_)
            ...
            if (found.leaf_->as_leaf()->which_ == detail::leaf_node_t<T>::which::vec) {
                ...
                if (inserted_size <= v.capacity() ||
                    (allocation_note == would_allocate && inserted_size <= detail::vec_insert_max)) {
