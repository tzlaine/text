...
collation_table make_save_load_table()
{
    ...
    std::string const table_str(data::mr::standard_collation_tailoring());
...
TEST(tailoring, mr_standard_000_001)
{
    ...
    {
    // greater than (or equal to, for =) preceeding cps
    auto const res = std::vector<uint32_t>{0x0915, 0x094d, 0x0937};
    auto const rel = std::vector<uint32_t>{0x091c, 0x094d, 0x091e};
    std::string const res_str = to_string(res);
    std::string const rel_str = to_string(rel);
    auto const res_view = as_utf32(res);
    auto const rel_view = as_utf32(rel);
