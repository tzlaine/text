...
{
    {
        ...
        boost::text::trie_map<std::vector<int>, int> trie_comp(text::less{});
    ...
    boost::text::trie_map<std::vector<int>, int> trie_0(elements);
    boost::text::trie_map<std::vector<int>, int> trie_comp(elements, text::less{});
    ...
    {
        boost::text::trie_map<std::vector<int>, int> other(
            {{{0, 1, 3}, 13}, {{0}, 17}, {{0, 1, 2}, 19}});
        boost::text::trie_map<std::vector<int>, int> trie(other);
...

    {
        boost::text::trie_map<std::vector<int>, int> other(
            {{{0, 1, 3}, 13}, {{0}, 17}, {{0, 1, 2}, 19}});
        boost::text::trie_map<std::vector<int>, int> trie(std::move(other));
...
{
    boost::text::trie_map<std::vector<int>, int> const trie_0 = {
        {{0, 1, 3}, 13}, {{0}, 17}, {{0, 1, 2}, 19}};
...
TEST(trie_map, iterators)
{
...

    {
        ...
        std::vector<text::trie_element<std::string, int>> copied_elements(
            trie.size());
        std::copy(trie.begin(), trie.end(), copied_elements.begin());

        std::vector<text::trie_element<std::string, int>>
