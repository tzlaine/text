...
TEST(algorithm, test_empty_view)
{
    ...
    text::string_view substr_view = tv(0, 0);
...
TEST(algorithm, test_empty_other_types)
{
    ...
    {
        text::string_view find_result = find_view(tv, str);
        EXPECT_TRUE(find_result.empty());
    }
    {
        text::string_view find_result = find_view(str, tv);
        EXPECT_TRUE(find_result.empty());
    }
    {
        text::string_view find_result = ::boost::text::find_view(str, str);
        EXPECT_TRUE(find_result.empty());
    }

    {
        text::string_view find_result = rfind_view(tv, str);
        EXPECT_TRUE(find_result.empty());
    }
    {
        text::string_view find_result = rfind_view(str, tv);
        EXPECT_TRUE(find_result.empty());
    }
    {
        text::string_view find_result = ::boost::text::rfind_view(str, str);
        EXPECT_TRUE(find_result.empty());
    }
...
TEST(algorithm, test_empty_view_constexpr)
{
    ...
    constexpr text::string_view find_view_view = find_view(tv, tv);
    static_assert(find_view_view == tv, "");

    constexpr int rfind_index = rfind(tv, tv);
    static_assert(rfind_index == 0, "");
    constexpr text::string_view rfind_view_view = rfind_view(tv, tv);
    ...
    constexpr text::string_view substr_view = tv(0, 0);
...
TEST(algorithm, test_view_view_compare_constexpr)
{
    ...
    constexpr text::string_view tv_b("b");
...
TEST(algorithm, test_view_view_find)
{
    text::string_view tv_empty;
    ...
    text::string_view tv_b("b");
    text::string_view tv_ab("ab");
    text::string_view tv_aab("aabaab");

    int find_a_empty = find(tv_a, tv_empty);
    EXPECT_EQ(find_a_empty, 0);
    int find_empty_a = find(tv_empty, tv_a);
    EXPECT_EQ(find_empty_a, -1);

    int find_a_ab = find(tv_a, tv_ab);
    EXPECT_EQ(find_a_ab, -1);
    int find_ab_a = find(tv_ab, tv_a);
    EXPECT_EQ(find_ab_a, 0);
    int find_a_a = find(tv_a, tv_a);
    EXPECT_EQ(find_a_a, 0);

    int find_b_ab = find(tv_b, tv_ab);
    EXPECT_EQ(find_b_ab, -1);
    int find_ab_b = find(tv_ab, tv_b);
    EXPECT_EQ(find_ab_b, 1);
    int find_b_b = find(tv_b, tv_b);
    EXPECT_EQ(find_b_b, 0);

    int find_a_b = find(tv_a, tv_b);
    EXPECT_EQ(find_a_b, -1);
    int find_b_a = find(tv_b, tv_a);
    EXPECT_EQ(find_b_a, -1);

    int find_ab_aab = find(tv_ab, tv_aab);
    EXPECT_EQ(find_ab_aab, -1);
    int find_aab_ab = find(tv_aab, tv_ab);
    EXPECT_EQ(find_aab_ab, 1);
}
...
TEST(algorithm, test_view_view_find_constexpr)
{
    ...
    constexpr text::string_view tv_b("b");
    constexpr text::string_view tv_ab("ab");
    constexpr text::string_view tv_aab("aabaab");
...
TEST(algorithm, test_view_view_find_view)
{
    text::string_view tv_empty;
    ...
    text::string_view find_a_empty = find_view(tv_a, tv_empty);
    EXPECT_EQ(find_a_empty.begin(), tv_a.begin());
    EXPECT_EQ(find_a_empty.size(), 0);
    text::string_view find_empty_a = find_view(tv_empty, tv_a);
    EXPECT_EQ(find_empty_a.begin(), nullptr);
    EXPECT_EQ(find_empty_a.size(), 0);

    text::string_view find_a_ab = find_view(tv_a, tv_ab);
    EXPECT_EQ(find_a_ab.begin(), nullptr);
    EXPECT_EQ(find_a_ab.size(), 0);
    text::string_view find_ab_a = find_view(tv_ab, tv_a);
    EXPECT_EQ(find_ab_a.begin(), tv_ab.begin());
    EXPECT_EQ(find_ab_a.size(), 1);
    text::string_view find_a_a = find_view(tv_a, tv_a);
    EXPECT_EQ(find_a_a.begin(), tv_a.begin());
    EXPECT_EQ(find_a_a.size(), 1);

    text::string_view find_b_ab = find_view(tv_b, tv_ab);
    EXPECT_EQ(find_b_ab.begin(), nullptr);
    EXPECT_EQ(find_b_ab.size(), 0);
    text::string_view find_ab_b = find_view(tv_ab, tv_b);
    EXPECT_EQ(find_ab_b.begin(), tv_ab.begin() + 1);
    EXPECT_EQ(find_ab_b.size(), 1);
    text::string_view find_b_b = find_view(tv_b, tv_b);
    EXPECT_EQ(find_b_b.begin(), tv_b.begin());
    EXPECT_EQ(find_b_b.size(), 1);

    text::string_view find_a_b = find_view(tv_a, tv_b);
    EXPECT_EQ(find_a_b.begin(), nullptr);
    EXPECT_EQ(find_a_b.size(), 0);
    text::string_view find_b_a = find_view(tv_b, tv_a);
    EXPECT_EQ(find_b_a.begin(), nullptr);
    EXPECT_EQ(find_b_a.size(), 0);

    text::string_view find_ab_aab = find_view(tv_ab, tv_aab);
    EXPECT_EQ(find_ab_aab.begin(), nullptr);
    EXPECT_EQ(find_ab_aab.size(), 0);
    text::string_view find_aab_ab = find_view(tv_aab, tv_ab);
    EXPECT_EQ(find_aab_ab.begin(), tv_aab.begin() + 1);
    EXPECT_EQ(find_aab_ab.size(), 2);
}
...
TEST(algorithm, test_view_view_find_first_of)
{
    text::string_view tv_empty;
    ...
    text::string_view tv_ab("ab");
    text::string_view tv_aab("aabaab");
    text::string_view tv_ca("ca");

    int find_first_of_a_empty = find_first_of(tv_a, tv_empty);
    EXPECT_EQ(find_first_of_a_empty, 0);
    int find_first_of_empty_a = find_first_of(tv_empty, tv_a);
    EXPECT_EQ(find_first_of_empty_a, -1);

    int find_first_of_a_ab = find_first_of(tv_a, tv_ab);
    EXPECT_EQ(find_first_of_a_ab, 0);
    int find_first_of_ab_a = find_first_of(tv_ab, tv_a);
    EXPECT_EQ(find_first_of_ab_a, 0);
    int find_first_of_a_a = find_first_of(tv_a, tv_a);
    EXPECT_EQ(find_first_of_a_a, 0);

    int find_first_of_b_ab = find_first_of(tv_b, tv_ab);
    EXPECT_EQ(find_first_of_b_ab, 0);
    int find_first_of_ab_b = find_first_of(tv_ab, tv_b);
    EXPECT_EQ(find_first_of_ab_b, 1);
    int find_first_of_b_b = find_first_of(tv_b, tv_b);
    EXPECT_EQ(find_first_of_b_b, 0);

    int find_first_of_a_b = find_first_of(tv_a, tv_b);
    EXPECT_EQ(find_first_of_a_b, -1);
    int find_first_of_b_a = find_first_of(tv_b, tv_a);
    EXPECT_EQ(find_first_of_b_a, -1);

    int find_first_of_ab_aab = find_first_of(tv_ab, tv_aab);
    EXPECT_EQ(find_first_of_ab_aab, 0);
    int find_first_of_aab_ab = find_first_of(tv_aab, tv_ab);
    EXPECT_EQ(find_first_of_aab_ab, 0);

    int find_first_of_ca_aab = find_first_of(tv_ca, tv_aab);
    EXPECT_EQ(find_first_of_ca_aab, 1);
    int find_first_of_aab_ca = find_first_of(tv_aab, tv_ca);
    EXPECT_EQ(find_first_of_aab_ca, 0);
}
...
TEST(algorithm, test_view_view_find_first_of_constexpr)
{
    ...
    constexpr text::string_view tv_ab("ab");
    constexpr text::string_view tv_aab("aabaab");
    constexpr text::string_view tv_ca("ca");
...
TEST(algorithm, test_view_view_find_first_not_of)
{
    text::string_view tv_empty;
    ...
    int find_first_not_of_a_empty = find_first_not_of(tv_a, tv_empty);
    EXPECT_EQ(find_first_not_of_a_empty, 0);
    int find_first_not_of_empty_a = find_first_not_of(tv_empty, tv_a);
    EXPECT_EQ(find_first_not_of_empty_a, -1);

    int find_first_not_of_a_ab = find_first_not_of(tv_a, tv_ab);
    EXPECT_EQ(find_first_not_of_a_ab, -1);
    int find_first_not_of_ab_a = find_first_not_of(tv_ab, tv_a);
    EXPECT_EQ(find_first_not_of_ab_a, 1);
    int find_first_not_of_a_a = find_first_not_of(tv_a, tv_a);
    EXPECT_EQ(find_first_not_of_a_a, -1);

    int find_first_not_of_b_ab = find_first_not_of(tv_b, tv_ab);
    EXPECT_EQ(find_first_not_of_b_ab, -1);
    int find_first_not_of_ab_b = find_first_not_of(tv_ab, tv_b);
    EXPECT_EQ(find_first_not_of_ab_b, 0);
    int find_first_not_of_b_b = find_first_not_of(tv_b, tv_b);
    EXPECT_EQ(find_first_not_of_b_b, -1);

    int find_first_not_of_a_b = find_first_not_of(tv_a, tv_b);
    EXPECT_EQ(find_first_not_of_a_b, 0);
    int find_first_not_of_b_a = find_first_not_of(tv_b, tv_a);
    EXPECT_EQ(find_first_not_of_b_a, 0);

    int find_first_not_of_ab_aab = find_first_not_of(tv_ab, tv_aab);
    EXPECT_EQ(find_first_not_of_ab_aab, -1);
    int find_first_not_of_aab_ab = find_first_not_of(tv_aab, tv_ab);
    EXPECT_EQ(find_first_not_of_aab_ab, -1);

    int find_first_not_of_ca_aab = find_first_not_of(tv_ca, tv_aab);
    EXPECT_EQ(find_first_not_of_ca_aab, 0);
    int find_first_not_of_aab_ca = find_first_not_of(tv_aab, tv_ca);
    EXPECT_EQ(find_first_not_of_aab_ca, 2);
}
...
TEST(algorithm, test_view_view_rfind)
{
    text::string_view tv_empty;
    ...
    int rfind_a_empty = rfind(tv_a, tv_empty);
    EXPECT_EQ(rfind_a_empty, 1);
    int rfind_empty_a = rfind(tv_empty, tv_a);
    EXPECT_EQ(rfind_empty_a, -1);

    int rfind_a_ab = rfind(tv_a, tv_ab);
    EXPECT_EQ(rfind_a_ab, -1);
    int rfind_ab_a = rfind(tv_ab, tv_a);
    EXPECT_EQ(rfind_ab_a, 0);
    int rfind_a_a = rfind(tv_a, tv_a);
    EXPECT_EQ(rfind_a_a, 0);

    int rfind_b_ab = rfind(tv_b, tv_ab);
    EXPECT_EQ(rfind_b_ab, -1);
    int rfind_ab_b = rfind(tv_ab, tv_b);
    EXPECT_EQ(rfind_ab_b, 1);
    int rfind_b_b = rfind(tv_b, tv_b);
    EXPECT_EQ(rfind_b_b, 0);

    int rfind_a_b = rfind(tv_a, tv_b);
    EXPECT_EQ(rfind_a_b, -1);
    int rfind_b_a = rfind(tv_b, tv_a);
    EXPECT_EQ(rfind_b_a, -1);

    int rfind_ab_aab = rfind(tv_ab, tv_aab);
    EXPECT_EQ(rfind_ab_aab, -1);
    int rfind_aab_ab = rfind(tv_aab, tv_ab);
    EXPECT_EQ(rfind_aab_ab, 4);
}
...
TEST(algorithm, test_view_view_rfind_view)
{
    text::string_view tv_empty;
    ...
    text::string_view find_a_empty = rfind_view(tv_a, tv_empty);
    EXPECT_EQ(find_a_empty.begin(), tv_a.end());
    EXPECT_EQ(find_a_empty.size(), 0);
    text::string_view find_empty_a = rfind_view(tv_empty, tv_a);
    EXPECT_EQ(find_empty_a.begin(), nullptr);
    EXPECT_EQ(find_empty_a.size(), 0);

    text::string_view find_a_ab = rfind_view(tv_a, tv_ab);
    EXPECT_EQ(find_a_ab.begin(), nullptr);
    EXPECT_EQ(find_a_ab.size(), 0);
    text::string_view find_ab_a = rfind_view(tv_ab, tv_a);
    EXPECT_EQ(find_ab_a.begin(), tv_ab.begin());
    EXPECT_EQ(find_ab_a.size(), 1);
    text::string_view find_a_a = rfind_view(tv_a, tv_a);
    EXPECT_EQ(find_a_a.begin(), tv_a.begin());
    EXPECT_EQ(find_a_a.size(), 1);

    text::string_view find_b_ab = rfind_view(tv_b, tv_ab);
    EXPECT_EQ(find_b_ab.begin(), nullptr);
    EXPECT_EQ(find_b_ab.size(), 0);
    text::string_view find_ab_b = rfind_view(tv_ab, tv_b);
    EXPECT_EQ(find_ab_b.begin(), tv_ab.begin() + 1);
    EXPECT_EQ(find_ab_b.size(), 1);
    text::string_view find_b_b = rfind_view(tv_b, tv_b);
    EXPECT_EQ(find_b_b.begin(), tv_b.begin());
    EXPECT_EQ(find_b_b.size(), 1);

    text::string_view find_a_b = rfind_view(tv_a, tv_b);
    EXPECT_EQ(find_a_b.begin(), nullptr);
    EXPECT_EQ(find_a_b.size(), 0);
    text::string_view find_b_a = rfind_view(tv_b, tv_a);
    EXPECT_EQ(find_b_a.begin(), nullptr);
    EXPECT_EQ(find_b_a.size(), 0);

    text::string_view find_ab_aab = rfind_view(tv_ab, tv_aab);
    EXPECT_EQ(find_ab_aab.begin(), nullptr);
    EXPECT_EQ(find_ab_aab.size(), 0);
    text::string_view find_aab_ab = rfind_view(tv_aab, tv_ab);
    EXPECT_EQ(find_aab_ab.begin(), tv_aab.begin() + 4);
    EXPECT_EQ(find_aab_ab.size(), 2);
}
...
TEST(algorithm, test_view_view_rfind_view_constexpr)
{
    ...
    constexpr text::string_view find_a_empty = rfind_view(tv_a, tv_empty);
    static_assert(find_a_empty.begin() == tv_a.end(), "");
    static_assert(find_a_empty.size() == 0, "");
    constexpr text::string_view find_empty_a = rfind_view(tv_empty, tv_a);
    static_assert(find_empty_a.begin() == nullptr, "");
    static_assert(find_empty_a.size() == 0, "");

    constexpr text::string_view find_a_ab = rfind_view(tv_a, tv_ab);
    static_assert(find_a_ab.begin() == nullptr, "");
    static_assert(find_a_ab.size() == 0, "");
    constexpr text::string_view find_ab_a = rfind_view(tv_ab, tv_a);
    static_assert(find_ab_a.begin() == tv_ab.begin(), "");
    static_assert(find_ab_a.size() == 1, "");
    constexpr text::string_view find_a_a = rfind_view(tv_a, tv_a);
    static_assert(find_a_a.begin() == tv_a.begin(), "");
    static_assert(find_a_a.size() == 1, "");

    constexpr text::string_view find_b_ab = rfind_view(tv_b, tv_ab);
    static_assert(find_b_ab.begin() == nullptr, "");
    static_assert(find_b_ab.size() == 0, "");
    constexpr text::string_view find_ab_b = rfind_view(tv_ab, tv_b);
    static_assert(find_ab_b.begin() == tv_ab.begin() + 1, "");
    static_assert(find_ab_b.size() == 1, "");
    constexpr text::string_view find_b_b = rfind_view(tv_b, tv_b);
    static_assert(find_b_b.begin() == tv_b.begin(), "");
    static_assert(find_b_b.size() == 1, "");

    constexpr text::string_view find_a_b = rfind_view(tv_a, tv_b);
    static_assert(find_a_b.begin() == nullptr, "");
    static_assert(find_a_b.size() == 0, "");
    constexpr text::string_view find_b_a = rfind_view(tv_b, tv_a);
    static_assert(find_b_a.begin() == nullptr, "");
    static_assert(find_b_a.size() == 0, "");

    constexpr text::string_view find_ab_aab = rfind_view(tv_ab, tv_aab);
    static_assert(find_ab_aab.begin() == nullptr, "");
    static_assert(find_ab_aab.size() == 0, "");
    constexpr text::string_view find_aab_ab = rfind_view(tv_aab, tv_ab);
...
TEST(algorithm, test_view_view_find_last_of)
{
    text::string_view tv_empty;
    ...
    int find_last_of_a_empty = find_last_of(tv_a, tv_empty);
    EXPECT_EQ(find_last_of_a_empty, 1);
    int find_last_of_empty_a = find_last_of(tv_empty, tv_a);
    EXPECT_EQ(find_last_of_empty_a, -1);

    int find_last_of_a_ab = find_last_of(tv_a, tv_ab);
    EXPECT_EQ(find_last_of_a_ab, 0);
    int find_last_of_ab_a = find_last_of(tv_ab, tv_a);
    EXPECT_EQ(find_last_of_ab_a, 0);
    int find_last_of_a_a = find_last_of(tv_a, tv_a);
    EXPECT_EQ(find_last_of_a_a, 0);

    int find_last_of_b_ab = find_last_of(tv_b, tv_ab);
    EXPECT_EQ(find_last_of_b_ab, 0);
    int find_last_of_ab_b = find_last_of(tv_ab, tv_b);
    EXPECT_EQ(find_last_of_ab_b, 1);
    int find_last_of_b_b = find_last_of(tv_b, tv_b);
    EXPECT_EQ(find_last_of_b_b, 0);

    int find_last_of_a_b = find_last_of(tv_a, tv_b);
    EXPECT_EQ(find_last_of_a_b, -1);
    int find_last_of_b_a = find_last_of(tv_b, tv_a);
    EXPECT_EQ(find_last_of_b_a, -1);

    int find_last_of_ab_aab = find_last_of(tv_ab, tv_aab);
    EXPECT_EQ(find_last_of_ab_aab, 1);
    int find_last_of_aab_ab = find_last_of(tv_aab, tv_ab);
    EXPECT_EQ(find_last_of_aab_ab, 5);

    int find_last_of_ca_aab = find_last_of(tv_ca, tv_aab);
    EXPECT_EQ(find_last_of_ca_aab, 1);
    int find_last_of_aab_ca = find_last_of(tv_aab, tv_ca);
    EXPECT_EQ(find_last_of_aab_ca, 4);
}
...
TEST(algorithm, test_view_view_find_last_not_of)
{
    text::string_view tv_empty;
    ...
    int find_last_not_of_a_empty = find_last_not_of(tv_a, tv_empty);
    EXPECT_EQ(find_last_not_of_a_empty, 1);
    int find_last_not_of_empty_a = find_last_not_of(tv_empty, tv_a);
    EXPECT_EQ(find_last_not_of_empty_a, -1);

    int find_last_not_of_a_ab = find_last_not_of(tv_a, tv_ab);
    EXPECT_EQ(find_last_not_of_a_ab, -1);
    int find_last_not_of_ab_a = find_last_not_of(tv_ab, tv_a);
    EXPECT_EQ(find_last_not_of_ab_a, 1);
    int find_last_not_of_a_a = find_last_not_of(tv_a, tv_a);
    EXPECT_EQ(find_last_not_of_a_a, -1);

    int find_last_not_of_b_ab = find_last_not_of(tv_b, tv_ab);
    EXPECT_EQ(find_last_not_of_b_ab, -1);
    int find_last_not_of_ab_b = find_last_not_of(tv_ab, tv_b);
    EXPECT_EQ(find_last_not_of_ab_b, 0);
    int find_last_not_of_b_b = find_last_not_of(tv_b, tv_b);
    EXPECT_EQ(find_last_not_of_b_b, -1);

    int find_last_not_of_a_b = find_last_not_of(tv_a, tv_b);
    EXPECT_EQ(find_last_not_of_a_b, 0);
    int find_last_not_of_b_a = find_last_not_of(tv_b, tv_a);
    EXPECT_EQ(find_last_not_of_b_a, 0);

    int find_last_not_of_ab_aab = find_last_not_of(tv_ab, tv_aab);
    EXPECT_EQ(find_last_not_of_ab_aab, -1);
    int find_last_not_of_aab_ab = find_last_not_of(tv_aab, tv_ab);
    EXPECT_EQ(find_last_not_of_aab_ab, -1);

    int find_last_not_of_ca_aab = find_last_not_of(tv_ca, tv_aab);
    EXPECT_EQ(find_last_not_of_ca_aab, 0);
    int find_last_not_of_aab_ca = find_last_not_of(tv_aab, tv_ca);
    EXPECT_EQ(find_last_not_of_aab_ca, 5);
}
