...
TEST(rope_view_algorithm, word_break)
{
    ...
    rope cps_("\x61\x5f\x61\x2e\x3a\x61");
    rope_view cps = cps_;
...
TEST(rope_view_algorithm, sentence_break)
{
    ...
    rope cps_("\x65\x74\x63\x2e\xe5\xae\x83");
    rope_view cps = cps_;
...
TEST(rope_view_algorithm, paragraph_break)
{
    // ÷ 0065 × 0074 × 0063 × 002E ÷ 5B83 ÷
    // ÷ [0.2] LATIN SMALL LETTER E (Lower) × [998.0] LATIN SMALL LETTER T (Lower) × [998.0] LATIN SMALL LETTER C (Lower) × [998.0] FULL STOP (ATerm) ÷ [11.0] CJK UNIFIED IDEOGRAPH-5B83 (OLetter) ÷ [0.3]
    rope cps_("\x65\x74\x63\x2e\xe5\xae\x83");
...
TEST(rope_view_algorithm, bidi)
{
    ...
    rope cps_(to_string(cp_vec.begin(), cp_vec.end()));
    rope_view cps = cps_;
...
TEST(rope_view_algorithm, coll_search_default)
{
    rope const haystack_1_("NOW AN FOWE\220ER ANNMAN THE ANPANMANEND");
    rope const needle_1_("ANPANMAN");
    rope const needle_2_("MAN THE");
    rope const needle_3_("WE\220ER");
    rope const needle_4_("NOW ");
    rope const needle_5_("NEND");
    rope const needle_6_("NOT FOUND");
    rope const needle_7_("NOT FO\340ND");

    rope const haystack_2_("ABC ABCDAB ABCDABCDABDE");
    rope const needle_11_("ABCDABD");

    rope const haystack_3_("abra abracad abracadabra");
    rope const needle_12_("abracadabra");

    rope_view haystack_1 = haystack_1_;
    rope_view needle_1 = needle_1_;
    rope_view needle_2 = needle_2_;
    rope_view needle_3 = needle_3_;
    rope_view needle_4 = needle_4_;
    rope_view needle_5 = needle_5_;
    rope_view needle_6 = needle_6_;
    rope_view needle_7 = needle_7_;

    rope_view haystack_2 = haystack_2_;
    rope_view needle_11 = needle_11_;

    rope_view haystack_3 = haystack_3_;
    rope_view needle_12 = needle_12_;
    ...
    do_search(table, haystack_1, rope_view(), 0, 0, __LINE__);
    do_search(table, rope_view(), needle_1, 0, 0, __LINE__);
}

TEST(rope_view_algorithm, coll_search_danish)
{
    rope const haystack_1_((char const *)u8"Danish aa ");
    rope const haystack_2_((char const *)u8"Danish aa");
    rope const haystack_3_((char const *)u8"Danish a");
    rope const haystack_4_((char const *)u8"Danish Å ");
    rope const haystack_5_((char const *)u8"Danish Å");
    rope const needle_1_((char const *)u8"Å");
    rope const needle_2_((char const *)u8"aa");
    rope const needle_3_((char const *)u8"AA");

    rope_view haystack_1 = haystack_1_;
    rope_view haystack_2 = haystack_2_;
    rope_view haystack_3 = haystack_3_;
    rope_view haystack_4 = haystack_4_;
    rope_view haystack_5 = haystack_5_;
    rope_view needle_1 = needle_1_;
    rope_view needle_2 = needle_2_;
    rope_view needle_3 = needle_3_;
...
TEST(rope_view_algorithm, coll_search_case_accents_and_punct)
{
...
TEST(rope_view_algorithm, case_mapping_)
{
    // Taken from case_mapping.cpp case 000.

    // 00DF; 00DF; 0053 0073; 0053 0053; # LATIN SMALL LETTER SHARP S
    rope cp_("\xc3\x9f"); // 00DF
    rope_view cp = cp_;
    ...
    { // to_title
        rope from_("\x61\x61\x61\xc3\x9f");
        rope_view from = from_;
