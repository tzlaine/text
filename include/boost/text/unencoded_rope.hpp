...
namespace boost { namespace text {
    ...
    // TODO: Add CharRange interface as in string.
...
namespace boost {
    namespace text {
        ...
        inline unencoded_rope & unencoded_rope::
        operator=(unencoded_rope_view rv)
        {
        ...
        inline unencoded_rope & unencoded_rope::erase(unencoded_rope_view rv)
        {
            ...
            if (string_insertion insertion = mutable_insertion_leaf(
                ...
                insertion.string_->erase((*insertion.string_)(
                    insertion.found_.offset_,
                    insertion.found_.offset_ + rv_size));
            } else {
                ptr_ = btree_erase(
                    ptr_,
                    rope_ref.lo_,
            ...
            unencoded_rope_view old_substr, unencoded_rope_view rv)
        {
            ...
            if (self_reference(rv)) {
                ...
                unencoded_rope_view::rope_ref rope_ref = rv.ref_.r_;
                rv =
                    unencoded_rope_view(extra_rope, rope_ref.lo_, rope_ref.hi_);
            }
            ...
            -> detail::char_iter_ret_t<unencoded_rope &, CharIter>
        {
            ...
            return replace(
                old_first, old_first + old_substr.size(), first, last);
        }
        ...
        inline unencoded_rope::const_iterator unencoded_rope::end() const
            noexcept
        {
        ...
        inline unencoded_rope::const_iterator
        end(unencoded_rope const & r) noexcept
        {
            return r.end();
        }
        ...
        /** Creates a new unencoded_rope object that is the concatenation of r
           and rv. */
        inline unencoded_rope
        operator+(unencoded_rope r, unencoded_rope_view rv)
        {
        ...
        /** Creates a new unencoded_rope object that is the concatenation of rv
           and r. */
        inline unencoded_rope
        operator+(unencoded_rope_view rv, unencoded_rope r)
        {
        ...
        inline unencoded_rope_view::const_iterator
        unencoded_rope_view::end() const noexcept
        {
            switch (which_) {
            case which::r:
                return const_iterator(
        ...
        namespace detail {
            ...
            template<typename CharIter>
            int mismatch_compare(
                unencoded_rope_view rv, CharIter rhs_first, CharIter rhs_last)
            {
                if (rv.empty())
                    return rhs_first == rhs_last ? 0 : -1;

                auto const iters = algorithm::mismatch(
                    rv.begin(), rv.end(), rhs_first, rhs_last);
        ...
        begin(unencoded_rope_view rv) noexcept
        {
            return rv.begin();
        }
        inline unencoded_rope_view::iterator
        end(unencoded_rope_view rv) noexcept
        {
            return rv.end();
        }
