...
any_non_reset_option_regex = re.compile(r'\[ *(?!before|first|last)[^\]]+\](?: *\])?')
reorder_regex = re.compile(r'\[ *reorder([^\]]+) *\]')
suppress_regex = re.compile(r'\[ *suppressContractions(?: +([^ \]]+))+ *\]')
relation_regex = re.compile(r' *(<\*|<<\*|<<<\*|<<<<\*|=\*|<|<<|<<<|<<<<|=) *([^=<]+)')
...
def cp_end(string, first, last):
    keep_trying = True
    while keep_trying:
        try:
            string[first:last].decode('UTF-8', 'strict')
            keep_trying = False
        except:
            last += 1
    return last

def expand_stars(rel):
    if '*' in rel[0]:
        op = rel[0][:-1]
        retval = []
        relations = rel[1].replace("'", '')
        i = 0
        while i < len(relations):
            if relations[i] == '-':
                # Just do the first and last element of each range; that's
                # good enough too.
                i += 1
                continue
            last = cp_end(relations, i, i + 1)
            retval.append((op, relations[i:last]))
            i = last
        return retval
    return [rel]

def make_rule_test(rule):
    retval = ''
    reset = rule[0].replace("'", '').replace('\\', '\\\\').strip()
    try:
        reset.decode('UTF-8', 'strict')
        reset = reset.encode('raw_unicode_escape')
    except:
        return retval
    for rel in rule[1:]:
        relation = rel[1].replace("'", '').replace('\\', '\\\\').strip()
        try:
            relation.decode('UTF-8', 'strict')
            relation = relation.encode('raw_unicode_escape')
        except:
            continue
        op = rel[0]
        if op == '=':
            retval += '''\
    EXPECT_EQ(text::collate("{0}"_sv, "{1}"_sv, text::collation_strength::quaternary), 0);

'''.format(reset, relation)
        elif op[0] == '<':
            ...
            if op == '<':
                ...
                strength = 'secondary'
                second_strength = 'primary'
            elif op == '<<<':
                strength = 'tertiary'
                second_strength = 'secondary'
            elif op == '<<<<':
                strength = 'quaternary'
                second_strength = 'tertiary'
            retval += '''\
    EXPECT_EQ(text::collate("{0}"_sv, "{1}"_sv, text::collation_strength::{2}), -1);
'''.format(reset, relation, strength)
            if second_strength != None:
                retval += '''\
    EXPECT_EQ(text::collate("{0}"_sv, "{1}"_sv, text::collation_strength::{2}), 0);
'''.format(reset, relation, second_strength)
            retval += '\n'
        else:
            raise Exception('Unknown tailoring rule op!')
        reset = relation
    return retval
...
for k0,v0 in sorted(tailorings_by_file.items()):
    ...
    for k,v in sorted(v0.items()):
        ...
        v_without_options = any_non_reset_option_regex.sub('', v)
        reset_lines = map(lambda x: x.strip(), v_without_options.split('&'))
        reset_lines = filter(lambda x: len(x), reset_lines)
        rules = []
        for line in reset_lines:
            ...
            if i < 0:
                ...
                if i < 0:
                    continue
            reset = [line[:i]]
            if 'before' in reset[0] or 'first' in reset[0] or 'last' in reset[0]:
                continue # TODO: Support these later.
            for match in relation_regex.finditer(line[i:]):
                reset += expand_stars(match.groups())
            rules.append(reset)
        for rule in rules:
            tailoring_rule_tests += make_rule_test(rule)
        if args.tests:
            f = open('tailoring_rules_{0}_{1}.cpp'.format(k0, k), 'w')
            f.write(tailoring_rules_form.format(k0, k, tailoring_rule_tests))
