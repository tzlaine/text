...
static const std::unordered_map<uint32_t, collation_elements> g_collation_element_table = {{
{0}
}};

namespace detail {{
    collation_elements collation_impl_singleton_{1}(uint32_t cp) noexcept
    {{
        auto const it = g_collation_element_table.find(cp);
...
def add_canonical_closure(fcc_ucet):
    ...
    return (new_ucet, median_single_cp_value)
...
(fcc_ucet, median_single_cp_value) = add_canonical_closure(fcc_ucet)
...
item_strings = map(
    lambda x : '{}, {}'.format(hex(x[0][0]), ces_to_vec(x[1])),
...
item_strings = map(
    ...
    filter(lambda x: len(x[0]) != 1, fcc_ucet.items())
)
decompositions_map = '    { ' + ' },\n    { '.join(item_strings) + ' },\n'
cpp_file = open('collation_data_3.cpp', 'w')
cpp_file.write(multiple_cp_key_file_form.format(decompositions_map))
