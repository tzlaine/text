...
namespace {
    ...
    boost::text::string_view string_views[14] = {make_string_view(0),
                                                 make_string_view(1),
                                                 make_string_view(2),
                                                 make_string_view(3),
                                                 make_string_view(4),
    ...
    boost::text::string make_string(int i)
    {
        return boost::text::string(std_strings[i]);
    }

    boost::text::string strings[14] = {make_string(0),
                                       make_string(1),
                                       make_string(2),
                                       make_string(3),
                                       make_string(4),
    ...
    boost::text::unencoded_rope make_unencoded_rope(int i)
    {
        ...
        while (size) {
            ...
            retval.insert(
                retval.size(),
                boost::text::string(boost::text::repeat(".", this_chunk_size)));
            size -= this_chunk_size;
        }
        return retval;
    }

    boost::text::unencoded_rope unencoded_ropes[14] = {make_unencoded_rope(0),
